...
// this file have rickey vad implemented with start recording button 
// start recording button will held up for 5 seconds silence if not spoken
// if spoken it will last for 0.5 seconds more 
  ...
  //@ts-ignore
  let {SESSION_ID,setData,setMsgLoading,audioArr,audioUrlFlag,audioUrlRef,msgLoading} = useData()
...
function playAudio2(url:string){
  console.log(audioPlayerRef,url)
  audioPlayerRef.current.src = `data:audio/mpeg;base64,${url}` 
    ...
    reader.onloadend = ()=>{
     ...
     fetch(url,{
        ...
        cache:'default',}).then(res=>{
          ...
          console.log(result)
        setMsgLoading(false)
        playAudio(result.audio_string)
         //playAudio2(result.audio_string)
        })
  ...
  function startMediaRecorder(stream:MediaStream,time:number){
   ...
   useEffect(()=>{
    ...
    useEffect(()=>{
      //console.log('i am current user at cues2',currentUser)
    ...
    <div className="App">
        ...
        <div className='recording'>
          <button className='recordBtn'
            ...
            >{recordingOn? `listening...`:`Ask Vitt`}
          </button>
        </div>
